...
void Analyzer::branchesElectrons(TTree* tree) {
...
void Analyzer::fillElectrons(const edm::Event &e, const edm::EventSetup &es, math::XYZPoint &pv) {
  ...
  if(doCalib_){
    ...
    if (!calibelectronHandle.isValid()) {
      edm::LogWarning("Analyzer") << "no calibrated pat::Electrons in event";
      return;
    }
  ...
  if (!electronHandle.isValid() && !isAOD_) {
    edm::LogWarning("Analyzer") << "no pat::Electrons in event";
    return;
  }
  ...
  for (edm::View<pat::Electron>::const_iterator iEle = electronHandle->begin(); iEle != electronHandle->end(); ++iEle) {
    ...
    */
    ...
    if (isAOD_) {
      ...
      eleGSFMissHits  .push_back(iEle->gsfTrack()->hitPattern().numberOfLostHits(reco::HitPattern::MISSING_INNER_HITS));
	...
	eleGSFMissHits  .push_back((*igsf)->hitPattern().numberOfLostHits(reco::HitPattern::MISSING_INNER_HITS));
