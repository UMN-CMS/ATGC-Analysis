...
#include "ATGC-Analysis/Analyzer/interface/GEDPhoIDTools.h"
...
double randomConeIso(double eta, edm::Handle<std::vector<pat::PackedCandidate>>& pfcands, const reco::Vertex& vertex,
	...
	while(overlap and attempt<40){
		...
		for (edm::View<pat::Photon>::const_iterator p = photons->begin(); p != photons->end(); ++p) {
		  if(p->pt() > 10 and reco::deltaR(eta, randomPhi, p->eta(), p->phi()) < 0.6) overlap = true;
		}
		...
		for (edm::View<pat::Jet>::const_iterator p = jets->begin(); p != jets->end(); ++p) {
		  if(p->pt() > 20 and reco::deltaR(eta, randomPhi, p->eta(), p->phi()) < 0.6) overlap = true;
		}
	...
	for(auto& iCand : *pfcands){
	  ...
	  if(reco::deltaR(eta, randomPhi, iCand.eta(), iCand.phi()) > 0.3) continue;
...
void Analyzer::branchesPhotons(TTree* tree) {
...
void Analyzer::fillPhotons(const edm::Event& e, const edm::EventSetup& es) {
  ...
  */
  ...
  if(doCalib_){
    ...
    if (!calibphotonHandle.isValid()) {
      edm::LogWarning("Analyzer") << "no calibrated pat::Photons in event";
      return;
    }
    
  }//if(doCalib_)

  if (!photonHandle.isValid() && !isAOD_) {
    edm::LogWarning("Analyzer") << "no pat::Photons in event";
    return;
  }
  ...
  */
  ...
  cout<<"GED tool taking"<<endl;
  if (isAOD_)
    GEDIdTool = new GEDPhoIDTools(e);
  cout<<"Took GED tool"<<endl;
  */
  ...
  */
  ...
  //cout<<"Looping over photons"<<endl;
  for (edm::View<pat::Photon>::const_iterator iPho = photonHandle->begin(); iPho != photonHandle->end(); ++iPho) {
    ...
    //cout<<"photon SCE "<<(*iPho).superCluster()->energy()<<endl;
    ...
    */
    ...
    if (isAOD_) {
      ...
      for (size_t iv = 0; iv < recoPhotonHandle->size(); ++iv) {
        ...
        if (reco::deltaR(iPho->eta(), iPho->phi(), recophoRef2->eta(), recophoRef2->phi()) < 0.01) rightRecoPho = iv;
      }
    ...
    */
    ...
    //cout<<"Filled everything for photons "<<endl;
