...
void Analyzer::branchesGenInfo(TTree* tree, edm::Service<TFileService> &fs) {
...
void Analyzer::branchesGenPart(TTree* tree) {
...
void Analyzer::fillGenInfo(const edm::Event& e) {
  ...
  if (genEventInfoHandle.isValid()) {
    ...
    edm::LogWarning("Analyzer") << "no GenEventInfoProduct in event";
  ...
  if (genPileupHandle.isValid()) {
    ...
    edm::LogWarning("Analyzer") << "no PileupSummaryInfo in event";

}

void Analyzer::fillGenPart(const edm::Event& e) {
  ...
  if (!genParticlesHandle.isValid()) {
    edm::LogWarning("Analyzer") << "no reco::GenParticles in event";
    return;
  }
  ...
  for (vector<reco::GenParticle>::const_iterator ip = genParticlesHandle->begin(); ip != genParticlesHandle->end(); ++ip) {
    ...
    bool photonOrLepton =
      ...
      */
      (ip->pdgId() == 22 ) ||
      (status == 1 && abs(ip->pdgId()) == 11 ) || 
      (status == 1 && abs(ip->pdgId()) == 13 ) ||
      (status == 1 && (abs(ip->pdgId()) == 12 || abs(ip->pdgId()) == 14 || abs(ip->pdgId()) == 16)) ||
      (status == 1 && ( abs(ip->pdgId()) >= 11 && abs(ip->pdgId()) <= 16 ) && ip->pt() > 3.0)  ||
      (status < 10 && abs(ip->pdgId()) == 15 && ip->pt() > 3.0);
    ...
    if ( heavyParticle || photonOrLepton || quarks || newParticle ) {
      ...
      //std::cout<<"tmpStatus flag "<<tmpStatusFlag<<std::endl;
      ...
      if (!runOnSherpa_) {
	//std::cout<<"Running on sherpa "<<std::endl;
	...
	//std::cout<<"mcMomPhi:"<<mcMomPhi_<<std::endl;
      }//if (!runOnSherpa_)
      ...
      if (photonOrLepton) {
	//std::cout<<"getting isolations"<<std::endl;
	...
	//std::cout<<"got isolations"<<std::endl;
      } else {
